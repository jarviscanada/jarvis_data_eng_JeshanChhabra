...
public static ResponseStatusException getResponseStatusException(Exception ex){
    if(ex instanceof IllegalArgumentException){
        logger.debug("Invalid output",ex);
        return new ResponseStatusException(HttpStatus.BAD_REQUEST,ex.getMessage());

    }
    else{
        logger.error("Invalid output",ex);
        return new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR,"Internal error: please talk to the admin");
    }
}
